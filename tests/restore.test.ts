...
describe("durable thread restore", () => {
	...
	it("holds resume and stop until the restore pass has finished", async () => {
		const stub = makeStub();
		registerLookupTools(stub.api, runtime);
		const control = stub.tools.find((tool) => tool.name === "subagent_control");
		const stop = stub.tools.find((tool) => tool.name === "subagent_stop");
		...
		const pendingStop = runTool(stop, "stop", { id: "6" }, executionContext()).then((result: any) => {
			stopSettled = true;
			return result;
		});
		await new Promise((resolve) => setTimeout(resolve, 20));
		// Answering now would report parked work as missing: both tools resolve a
		// run id against threads restore has not installed yet.
		expect(controlSettled).toBe(false);
		expect(stopSettled).toBe(false);

		runtime.threads.set(5, restorableThread(5));
		runtime.threads.set(6, restorableThread(6));
		finishRestore();

		expect((await pendingControl).content[0].text).toContain("Resumed run #5");
		expect((await pendingStop).content[0].text).toContain("#6");
	});
});
